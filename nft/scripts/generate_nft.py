...
def generate_art(url, img_id):
    ...
    _70x70_resizedImgLInk, _500x500_resizedImgLInk = resizeImage(url, img_id)  #Resize the image for convenience

    img_id, code_link, nft_code = generateCode(img_id)  #Generate the code for the NFT
    main_img = Image.open(_500x500_resizedImgLInk)   #Open the resized image
    code_img = Image.open(code_link)          #Open the code image
    
    w_main, h_main = main_img.size                      #Get the width and Heigth of the main image after resizing
    ...
    return nft_save_link, code_link, nft_code, _70x70_resizedImgLInk
...
def resizeImage(url, img_id):
    ...
    if(isImage(url)):
        ...
        image_thumb_500x500 = os.path.join(site_img_display_folder,name_500x500)
